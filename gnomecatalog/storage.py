...
class Sqlite:
    ...
    def open(self, database):
        ...
        print("Create: %s" % self.database_tempfile)
    ...
    def save(self):
        self.commit()
        self.compress()
        print("Copiando %s into %s" %(self.database_tempfile, self.database))
...
#        print("SQL: ",sql)
        ...
        try:
            ...
            print("SQL error occurred: %s: %s" %(e.args[0], sql))
    ...
    def __update_database(self, version):
        print('Updating to version %s' % self.version)
        if version == '3.0':
...
#        icon = disk.get_icon().get_pixels()
    ...
    def __insert_file(self, iddisk, idparent, file):
        self.__sql("INSERT into files (id, iddisk, idparent, name, size, path, mime, type) "
                   "VALUES (NULL, " + str(iddisk) +", " + str(idparent) + " ,'" + self._check(file.name) + "', " + str(file.size) + ", '" +  self._check(file.path) + "', '" + file.mime + "', '" + str(file.type) + "')")
        ...
        if file.meta:
            for key, val in file.meta.items():
                ...
                try:
                    ...
                    print("ERROR: Unable to insert metadata: %s (FIXME@mzimen!)" % exp)
        ...
        if config.get("thumbnails") == "True":
            try:
                ...
                print("ERROR: Unable to save thumbnail: %s (FIXME@mzimen!)" % exp)
